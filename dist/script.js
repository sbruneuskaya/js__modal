    ...
    elem.forEach(function (item, i) {
      item.addEventListener(event, function () {
        switch (item.nodeName) {
          // когда использую nodeName названия приходят в верхнем регистре
          case "SPAN":
            state[prop] = i;
            break;
...
var forms = function forms(state) {
  ...
  inputs = document.querySelectorAll('input'); // обработка ввода данных(чтобы пользователь мог ввести в инпут телефона только цифры)

  Object(_checkNumInputs__WEBPACK_IMPORTED_MODULE_5__["default"])('input[name="user_phone"]'); // для оповещения пользователя об отправке формы
    ...
    return regeneratorRuntime.async(function postDate$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            document.querySelector('.status').textContent = message.loading; // с помощью await в переменную res будет записан промис, который вернется от сервера, если бы без async await, в res был бы андефайнд, потому что код бы пошелработать дальше не дождавшись ответа
  ...
  form.forEach(function (item) {
    item.addEventListener("submit", function (e) {
      e.preventDefault(); // создаем блок в котором будем показывать сообщение в зависимости от статуса

      var statusForm = document.createElement('div');
      statusForm.classList.add('status');
      item.appendChild(statusForm); // собираю все данные которые есть в форме, это и будет тело запроса
      ...
      postDate('http://sveta_server', formDate) // сейчас вернулся какой-то текст return await res.text()
      .then(function (res) {
        console.log(res);
        statusForm.textContent = message.success;
      }).catch(function () {
        return statusForm.textContent = message.error;
...
var images = function images() {
  ...
  imgPopup.classList.add('popup'); // помещаю в секцию созданный див

  workSection.appendChild(imgPopup);
  imgPopup.style.justifyContent = 'center';
  imgPopup.style.alignItems = 'center';
  imgPopup.style.display = 'none'; // помещаю в див большую картинку

  imgPopup.appendChild(bigImage);
  workSection.addEventListener('click', function (e) {
    e.preventDefault();
    var target = e.target; // проверяю поддерживает ли вообще событие таргет и содержит ли определенный класс
...
var modals = function modals() {
  ...
  function calcScroll() {
    ...
    document.body.appendChild(div); // div.offsetWidth---получаем полную ширину с бордерами и всеми входящими величинами
  ...
  bindModals('.popup_calc_profile_button', '.popup_calc_end', '.popup_calc_end_close', false); // showModalByTime('.popup', 60000)
};
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  hideTabContent();
  showTabContent(); // навешиваем обработчик события на место, которое объединят в себе все табы(родитель всех табов)

  header.addEventListener('click', function (e) {
    var target = e.target; // проверяем, что действительно кликнули в один из табов(пока не важно в какой). в if придет элемент с точкой, нужно ее убрать, потому что использую classList.contains, а replace первым параметром принимает регулярное выражение
...
var timer = function timer(id, deadline) {
  ...
  var getTimeRemaining = function getTimeRemaining(endTime) {
    ...
    var t = Date.parse(endTime) - Date.parse(new Date()); // t/1000-----переводим все миллисекунды в миллисекунды
  ...
  var setClock = function setClock(selector, endTime) {
    var timer = document.querySelector(selector),
        ...
        timeInterval = setInterval(updateClock, 1000); // запускаем функцию updateClock() раньше, до того как она запустится с помощью
    ...
    updateClock(); // определяем сколько времени осталось до дедлайна + эта функция каждую секунду возвращает новое значение
